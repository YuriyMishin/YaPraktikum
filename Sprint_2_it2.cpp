...
void TestExcludeStopWordsFromAddedDocumentContent() {
    ...
    server.SetStopWords("sasha uchitsya stroit dom i lubit gulyat s sobakoi"s);
    ...
    ASSERT_HINT(server.FindTopDocuments("uchitsya stroit dom i lubit gulyat"s).empty(),err_info); //Запрос, состоящий из части стоп-слов из разных документов, не найден
}
void TestExcludeMinusWordsFromFind() {
 
    SearchServer server;
    const string err_info="Finded doc included minus words";
    server.AddDocument(0, "sasha stroit dom i lubit gulyat s sobakoi", DocumentStatus::ACTUAL, {10, 2, 3, -5}); //добавленные документы не пересекаются
    server.AddDocument(1, "vasya uchitsya v shkole", DocumentStatus::ACTUAL, {1, 2, 3, -5});
    
    const auto found_docs0 = server.FindTopDocuments("sasha -uchitsya stroit dom i lubit gulyat s sobakoi"s);
    ASSERT_EQUAL_HINT(found_docs0.size(),1,err_info); //Запрос c минус словом, входящим в другой документ, находит только один с правильным id
    const Document& doc0 = found_docs0[0];
    ASSERT_EQUAL_HINT(doc0.id,0,err_info);
    
    ASSERT_HINT(server.FindTopDocuments("-vasya"s).empty(),err_info); //минус слово не найдено
...
void TestAveregeRating(){
    SearchServer server;
    const string err_info="Error average rating";
    const vector <int> rating_id0={8, 2, 3, -5};
    ...
    ASSERT_EQUAL_HINT(doc0.rating,2,err_info); // проверяем правильность расчета рейтинга, сравнивая с рассчитаным вручную
    
    const auto document1 = server.FindTopDocuments("vasya uchitsya v shkole");
    const Document& doc1 = document1[0];
    ASSERT_EQUAL_HINT(doc1.rating,0,err_info); // проверяем правильность расчета рейтинга, сравнивая с рассчитаным вручную == 0
}
...
void TestFilterPredicate(){
    ...
    ASSERT_EQUAL_HINT(document0.size(),1,err_info); // один документ со статусом ACTUAL id=0 добавили и один нашли
    const Document& doc0 = document0[0];
    ASSERT_EQUAL_HINT(doc0.id,0,err_info);
    ...
    ASSERT_EQUAL_HINT(document0.size(),1,err_info); // документ c id==1 нашелся единственный
    const Document& doc1 = document1[0];
    ASSERT_EQUAL_HINT(doc1.id,1,err_info);
    
    vector<Document> document2 = server.FindTopDocuments("vasya sasha serega alisa",[](int document_id, DocumentStatus status, int rating) { return rating > 0; }); //поиск по рейтингу
    for (vector<Document>::iterator it = document2.begin(); it < document2.end(); ++ it){
        ASSERT_HINT(((*it).rating > 0),err_info);   //Все найденные документы имеют средний рейтинг > 0
    }
...
void TestFilterStatus(){
    ...
    ASSERT_EQUAL_HINT(document0.size(),1,err_info); // один документ со статусом ACTUAL добавили и один нашли
    const Document& doc0 = document0[0];
    ASSERT_EQUAL_HINT(doc0.id,0,err_info);
    
    const auto document1 = server.FindTopDocuments("vasya sasha serega alisa",DocumentStatus::IRRELEVANT);
    ASSERT_EQUAL_HINT(document1.size(),1,err_info); // один документ со статусом IRRELEVANT добавили и один нашли
    const Document& doc1 = document1[0];
    ASSERT_EQUAL_HINT(doc1.id,1,err_info);
    
    const auto document2 = server.FindTopDocuments("vasya sasha serega alisa",DocumentStatus::BANNED);
    ASSERT_EQUAL_HINT(document2.size(),1,err_info); // один документ со статусом BANNED добавили и один нашли
    ...
    ASSERT_EQUAL_HINT(document3.size(),1,err_info); // один документ со статусом REMOVED добавили и один нашли
...
void TestRelevance(){
    ...
    // Расчет релевантности для запроса
    //IDF_word = ln(all_documents_count/documents_count_with_word); TF_word = count_word_in_document/count_all_word_in_document
    //Relevance = SUM (IDF_word*TF_word)
    // Расчет релевантности для запроса - ("sasha")
    //all_documents_count==4
    //IDF_sasha=ln(4/1) TF_sasha==1/8
    const double relevance_sasha = log(4)*0.125;
    const auto document0 = server.FindTopDocuments("sasha");  //Проверяем релевантность с посчитанной вручную
    ASSERT_EQUAL_HINT(document0.size(),1,err_info); // один документ в векторе
    const Document& doc0 = document0[0];
    ASSERT_HINT((abs(doc0.relevance - relevance_sasha) < 10e-6),err_info);
    
    //IDF_stroit=ln(4/1) TF_stroit==1/8
    // relevance_stroit == relevance_sasha
    const double relevance_sasha_stroit = relevance_sasha*2;
    const auto document1 = server.FindTopDocuments("sasha stroit");
    ASSERT_EQUAL_HINT(document1.size(),1,err_info); // один документ в векторе
    const Document& doc1 = document1[0];
    ASSERT_HINT((abs(doc1.relevance - relevance_sasha_stroit) < 10e-6),err_info);
    
    //Релевантность запроса соответствующего всему документу == relevance_sasha * количество слов в документе
    const double relevance_zapros_document = relevance_sasha*8;
    const auto document2 = server.FindTopDocuments("sasha stroit dom i lubit gulyat s sobakoi");
    ASSERT_EQUAL_HINT(document2.size(),1,err_info); // один документ в векторе
    const Document& doc2 = document2[0];
    ASSERT_HINT((abs(doc2.relevance - relevance_zapros_document) < 10e-6),err_info);
}
